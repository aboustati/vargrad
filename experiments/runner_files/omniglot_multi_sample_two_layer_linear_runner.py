...
LOSSES = ['score_function', 'variance_loss', 'concrete_relaxation', 'arm', 'rebar',
          'relax', 'tunable_rebar', 'controlled_score_function', 'relax_plus_rebar']

MODEL = 'omniglot_multi_sample_two_layer_linear'
...
if __name__ == '__main__':
    ...
    for loss, file in runs:
        ...
        try:
            if loss in ['relax', 'relax_plus_rebar']:
                for s in random.sample([1, 5], 2):
                    print(f"Running {loss} from {file} with {s} samples")
                    config['num_samples'] = s
                    exp = RunRelaxInstance(os.path.join(RESULTS_PATH, MODEL), config=config, loss=loss,
                                           dataset='omniglot', two_layer=True)
                    exp.run_and_save(prefix=f'num_samples_{s}')
                    print("Run finished")
            elif loss == 'variance_loss':
                for s in random.sample([2, 5, 10], 3):
                    print(f"Running {loss} from {file} with {s} samples")
                    config['num_samples'] = s
                ...
                for s in random.sample([1, 5, 10], 3):
                    if loss == 'tunable_rebar' and s == 10:
                        continue
                    print(f"Running {loss} from {file} with {s} samples")
                    config['num_samples'] = s
                    exp = RunInstance(os.path.join(RESULTS_PATH, MODEL), config=config, loss=loss,
                                      dataset='omniglot', two_layer=True)
                    exp.run_and_save(prefix=f'num_samples_{s}')
